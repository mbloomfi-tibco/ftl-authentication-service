...
keytool -genkeypair -alias auth-server -keyalg RSA -keysize 4096 -storetype PKCS12 -keystore src/main/resources/keystore-auth.p12 -validity 3650 -storepass "3q@v)dha{M:7A{8?" -dname "CN=localhost" -ext "SAN=IP:192.168.76.1,IP:127.0.0.1,IP:192.168.76.42,IP:192.168.76.43,IP:192.168.76.44,IP:192.168.1.128,DNS:messaging1.lxc,DNS:messaging2.lxc,DNS:messaging3.lxc"
...
keytool -alias auth-server -certresoq -keystore "src/main/resources/keystore-identity.p12" -storepass "3q@v)dha{M:7A{8?" -keyalg RSA  | keytool -alias intermediate -gencert -storepass password -keyalg RSA | keytool -alias auth-server -importcert -keyalg RSA -keystore "src/main/resources/keystore-identity.p12" -storepass "3q@v)dha{M:7A{8?" -noprompt -trustcacerts

echo "Export public and root certificates"
rm client-certificate/public-cert.pem
rm client-certificate/root-cert.pem
rm client-certificate/auth-public-cert.pem
...
cat client-certificate/public-cert.pem client-certificate/root-cert.pem >> client-certificate/auth-public-cert.pem
